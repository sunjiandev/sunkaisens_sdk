...
public class NgnMsrpSession extends NgnInviteSession {
	...
	public void setMsgType(String intype)
	{
		msg_type = intype;
	}
//sks add end.
	...
	public static NgnMsrpSession takeIncomingSession(NgnSipStack sipStack, MsrpSession session, SipMessage message){
        ...
        if (mediaType == NgnMediaType.Chat){
            ...
            String msgtype1 = message.getSipHeaderValue("MsgExt.msgType");
            //String msgtype2 = message.getSipHeaderParamValue("MsgExt","msgType");
            msrpSession.setMsgType(msgtype1);
        }
        else{
            String name = null;
            String type = null;
...
//		super.setToUri(toUri);
		//wangds modify error: the remote uri may be null.  2014.6.6
		if(InviteState.INCOMING == callState)
		{
	...
	public boolean accept() {
		if (super.getState() == InviteState.INCOMING
			...
			try {
				final File newFile = new File(mFilePath); ///sdcard/wiPhone/search_clear_pressed.png
				if (!newFile.exists()) {
					File parentFile = newFile.getParentFile(); ///sdcard/wiPhone
					parentFile.mkdirs();
				...
				mOutFileStream = new FileOutputStream(newFile.getAbsolutePath(), false); ///sdcard/wiPhone/search_clear_pressed.png
				//zhh add start
				if (mOutFileStream == null) {
					mFileName = newFile.getName();
					String path = Environment.getExternalStorageDirectory() + "/FileTransfer/";
					File dir = new File(path);
					if (!dir.exists()) {
						dir.mkdirs();
					}
					mOutFileStream = new FileOutputStream(path + mFileName, false); ///sdcard/wiPhone/search_clear_pressed.png
				}
				//zhh add end
			} catch (Exception e) {
				Log.e(TAG, e.toString()); //(java.lang.String) java.io.IOException: open failed: EINVAL (Invalid argument)
				hangUp();
				return false;
			}
		}
	...
	public boolean sendFile(String path) {
		if (NgnStringUtils.isNullOrEmpty(path) || !isFileExists(path)) { ///mnt/sdcard/DCIM/Camera/IMG_20140610_151900.jpg
	...
	static class NgnMsrpCallback extends MsrpCallback {
		...
		NgnMsrpCallback(NgnMsrpSession session) {
			super();
			mSession = session;
			mAppContext = NgnSKDroid.getInstence().getGlobleContext();
			mSessionId = -1;
		}
                ...
                case tmsrp_SEND:
                    {
                        ...
                        long read = 0;
                        if(clen == 0){
                            MyLog.d(TAG,"Empty MSRP message");
                            return;
                        }
                        ...
                        if(mSession.mPendingMessages != null && mSession.mPendingMessages.size() > 0) {
                            if(mSession.isConnected()){
                                for (PendingMessage pendingMsg : mSession.mPendingMessages){
                                    MyLog.d(TAG,"Sending pending message...");
