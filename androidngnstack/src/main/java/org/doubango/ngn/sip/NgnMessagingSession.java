...
public class NgnMessagingSession extends NgnSipSession {
	...
	private final static NgnObservableHashMap<Long, NgnMessagingSession> sSessions = new NgnObservableHashMap<Long, NgnMessagingSession>(
			true);
	private final INgnConfigurationService mConfigurationService;

	public static NgnMessagingSession takeIncomingSession(NgnSipStack sipStack,
			MessagingSession session, SipMessage sipMessage) {
		final String toUri = sipMessage == null ? null : sipMessage
				.getSipHeaderValue("f");
		NgnMessagingSession imSession = new NgnMessagingSession(sipStack,
				session, toUri);
		sSessions.put(imSession.getId(), imSession);
		return imSession;
	}

	public static NgnMessagingSession createOutgoingSession(
			NgnSipStack sipStack, String toUri) {
		synchronized (sSessions) {
			final NgnMessagingSession imSession = new NgnMessagingSession(
					sipStack, null, toUri);
			sSessions.put(imSession.getId(), imSession);
			return imSession;
		}
	...
	protected NgnMessagingSession(NgnSipStack sipStack,
			MessagingSession session, String toUri) {
		super(sipStack);
		mSession = session == null ? new MessagingSession(sipStack) : session;

		super.init();
		super.setSigCompId(sipStack == null ? "" : sipStack.getSigCompId());
	 ...
	 * 
	 * @param text
	 *            the text (utf-8) to send.
	 * @param SMSC
	 *            the address (PSI) of the SMS center
	...
	public boolean SendBinaryMessage(String text, String SMSC) {
		...
		if ((SMSCPhoneNumber = NgnUriUtils.getValidPhoneNumber(SMSC)) != null
				&& (dstPhoneNumber = NgnUriUtils.getValidPhoneNumber(dstSipUri)) != null) {
			MyLog.d(TAG, "NgnMessagingSession SendBinaryMessage line 135");
			super.setToUri(SMSC);
			...
			RPMessage rpMessage;
			// if(ServiceManager.getConfigurationService().getBoolean(CONFIGURATION_SECTION.RCS,
			// CONFIGURATION_ENTRY.HACK_SMS, false)){
			// rpMessage = SMSEncoder.encodeDeliver(++ScreenSMSCompose.SMS_MR,
			// SMSCPhoneNumber, dstPhoneNumber, new String(content));
			// session.addHeader("P-Asserted-Identity", SMSC);
			// }
			// else{
			rpMessage = SMSEncoder.encodeSubmit(++NgnMessagingSession.SMS_MR,
					SMSCPhoneNumber, dstPhoneNumber, text);
			MyLog.d(TAG, "SendBinaryMessage rpMessage:" + rpMessage);
			// }
			...
			long payloadLength = rpMessage.getPayload(payload,
					(long) payload.capacity());
			...
			MyLog.d(TAG, "NgnMessagingSession SendBinaryMessage line 162");
			Log.e(TAG, String.format("SMSC=%s or RemoteUri=%s is invalid",
					SMSC, dstSipUri));
	...
	public boolean sendGPSMessage(String text) {
		super.addHeader("APP_Type", NgnContentType.GPS_APP_TYPE);// IMS
																	// messaging/DDS-mode
																	// messaging
		super.addHeader("Content-Type",
				NgnContentType.GPS_CONTENT_TYPE_HEADER_COMMAND);// application/dds-gps+xml
	...
	public boolean sendGPSMessage_data(String text) {
		super.addHeader("APP_Type", NgnContentType.GPS_APP_TYPE); // IMS
																	// messaging/DDS-mode
																	// messaging
		super.addHeader("Content-Type",
				NgnContentType.GPS_CONTENT_TYPE_HEADER_DATA); // application/dds-gps-data+xml
	...
	// sks add 添加
	public boolean sendExTextMessage(String text, String contentType) {
		if (!NgnStringUtils.isNullOrEmpty(contentType)) {
			super.addHeader("Content-Type", contentType);
		} else {
			super.addHeader("Content-Type", "Message/CPIM");
			// super.addHeader("Content-Disposition","render");
			super.addHeader("APP_Type", " IMS messaging/Page-mode messaging");
		}
	...
	public boolean sendExTextMessage(String text) {
		return sendExTextMessage(text, null);
	}

	public boolean sendExWithListTextMessage(String text, String contentType) {
		if (!NgnStringUtils.isNullOrEmpty(contentType)) {
			super.addHeader("Content-Type", contentType);
		} else {
			super.addHeader("Content-Type",
			...
			super.addHeader("Content-Disposition", "render");
			super.addHeader("Require", "recipient-list-message");
			super.addHeader("APP_Type",
					" IMS messaging/Page-mode messaging/CHINA MOBILE");
		}
		byte[] bytes = text.getBytes();
		ByteBuffer payload = ByteBuffer.allocateDirect(bytes.length);
		payload.put(bytes);
	...
	public boolean sendExWithListTextMessage(String text) {
		return sendExWithListTextMessage(text, null);
	}
