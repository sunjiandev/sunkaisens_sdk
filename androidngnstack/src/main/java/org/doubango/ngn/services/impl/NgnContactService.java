...
import org.doubango.utils.AndroidUtils.PermissionSKD;
...
public class NgnContactService  extends NgnBaseService implements INgnContactService{
	...
	public boolean start() {
		...
		if(mLocalContactObserver == null && mLocalContactObserverLooper == null){
			new Thread(new Runnable() { // avoid locking calling thread
				@Override
				public void run() {
					MyLog.d(TAG, "Observer Looper enter()");
					Looper.prepare();
						...
						public void run() {
							mLocalContactObserver = new ContentObserver(handler) {
								...
								public void onChange(boolean selfChange) {
									...
									MyLog.d(TAG, "Native address book changed");
									load();
								}
							};
							NgnSKDroid.getInstence().getGlobleContext().getContentResolver().registerContentObserver(CommonDataKinds.Phone.CONTENT_URI,
									true, mLocalContactObserver);
						}
					});
					Looper.loop();// loop() until quit() is called
					MyLog.d(TAG, "Observer Looper exit()");
				}
			}).start();
		};
	...
	public boolean stop() {
		...
		try{
			if(mLocalContactObserver != null){
				NgnSKDroid.getInstence().getGlobleContext().getContentResolver().unregisterContentObserver(mLocalContactObserver);
	...
	public boolean load2(){
		...
		if(!AndroidUtils.hasPermission(PermissionSKD.READ_CONTACTS))
			return false;
		...
		try{
			...
			int id, type, photoId;
			final Resources res = NgnSKDroid.getInstence().getGlobleContext().getResources();
