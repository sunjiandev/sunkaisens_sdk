 ...
 * 					return;
 * 				}
 * 				MyLog.d(TAG, "This is an event for session number "+args.getSessionId());		
 ...
 * As its name says, it抯 you public visible identifier where you are willing to receive calls or any
 ...
 * barred if it doesn抰 appear in the associated URIs returned in the 200 OK REGISTER. <br />
 ...
 * You should never manually set this SIP header (P-Preferred-Identity); it抯 up to the stack.
 * 
...
public interface INgnSipService extends INgnBaseService {
     ...
     * @return
     */
    boolean isRegisteSessionConnected();
    /**
	...
	boolean ADHOC_Start(Context context);
	boolean ADHOC_Stop();
	boolean ADHOC_SetPcscfHost(String hostip);
}
