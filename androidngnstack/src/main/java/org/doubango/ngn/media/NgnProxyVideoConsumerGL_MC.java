 ...
 */
public class NgnProxyVideoConsumerGL_MC extends NgnProxyVideoConsumer{
	private static final String TAG = NgnProxyVideoConsumerGL_MC.class.getCanonicalName();
...
//	private static boolean mIsFirst = true;
	...
	protected NgnProxyVideoConsumerGL_MC(BigInteger id, ProxyVideoConsumer consumer){
    	super(id, consumer);
    	mConsumer = consumer;
...
//    	mIsFirst = true;
    	...
    	mWidth = NgnProxyVideoConsumerGL_MC.DEFAULT_VIDEO_WIDTH;
    	mHeight = NgnProxyVideoConsumerGL_MC.DEFAULT_VIDEO_HEIGHT;
    	mFps = NgnProxyVideoConsumerGL_MC.DEFAULT_VIDEO_FPS;
    }
    	...
    	synchronized(this){
	    	...
	    	if(mContext != null){
		    	if(mPreview == null || mPreview.isDestroyed()){
	    			...
	    			NgnAvcDecoder.getInstance().setNgnAveDecoderParameter(mWidth, mHeight,mFps,null);	
    ...
    private int startCallback(){
    	synchronized(this){
	    	Log.d(NgnProxyVideoConsumerGL_MC.TAG, "startCallback");
	    	super.mStarted = true;
	    	return 0;
    	}
    }
...
//				Log.d("",String.format("NgnAvcDecoder: [%d,%d] %d %d", mWidth,mHeight,(int)nCopiedSize,(int)nAvailableSize));
				if(recvLen >= (mWidth * mHeight * 3) >> 1)
				{
					mPreview.requestRender();
					return 0;
				}
					
				dataRecv = new byte[recvLen];
				mVideoFrame.rewind();
				mVideoFrame.get(dataRecv);
				
				if(dataRecv != null && dataOut != null 
						&& NgnAvcDecoder.getInstance() != null){
				
					int ret = NgnAvcDecoder.getInstance().offerDecoder(dataRecv, dataOut, recvLen);
					Log.d("",String.format("NgnAvcDecoder: recvLen=%d decodeLen=%d ,mVideoFrame=%d", recvLen,ret,mVideoFrame.capacity()));
					if(ret >= 0)
					{
						mVideoFrame.rewind();
						mVideoFrame.put(dataOut);
						mPreview.requestRender();
					}
				}else {
					MyLog.d(TAG, "dataRecv = "+dataRecv);
				}
    ...
    {
        final NgnProxyVideoConsumerGL_MC mConsumer;

        public NgnProxyVideoConsumerGLCallback(NgnProxyVideoConsumerGL_MC consumer){
        	super();
        	mConsumer = consumer;
        }
