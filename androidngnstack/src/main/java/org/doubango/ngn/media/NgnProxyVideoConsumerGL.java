...
public class NgnProxyVideoConsumerGL extends NgnProxyVideoConsumer {
	private static final String TAG = NgnProxyVideoConsumerGL.class
			.getCanonicalName();
	...
	public final View startPreview(Context context) {
		synchronized (this) {
			MyLog.d(TAG, "mPreview" + mPreview == null);
			...
			if (mContext != null) {
				if (mPreview == null || mPreview.isDestroyed()) {
					mPreview = new NgnProxyVideoConsumerGLPreview(mContext,
							super.mFullScreenRequired, mVideoFrame, mWidth,
							mHeight, mFps);
				}
			}
			return mPreview;
	...
	private int bufferCopiedCallback(long nCopiedSize, long nAvailableSize) {
		...
		long dataWidth = mConsumer.getDecodedWidth();
		long dataHeight = mConsumer.getDecodedHeight();
		...
		if (mPreview == null || !mPreview.isReady()) {
			...
			if (mTvHandler != null && GlobalSession.bSocketService == true) { // 发送大终端模拟视频输出消息
				...
				Bundle b = new Bundle();
				b.putInt("width", (int) frameWidth);
				b.putInt("height", (int) frameHeight);
				b.putByteArray("data", mVideoFrame.array());
				msg.setData(b);
				msg.sendToTarget();
			}
		...
		if (mVideoFrame == null || mWidth != frameWidth
				|| frameHeight != mHeight
				|| mVideoFrame.capacity() != nAvailableSize) {
			synchronized (mPreview) {
				MyLog.d(TAG, "VideoConsumer  frameW/H:"
						+ (frameWidth + "/" + frameHeight) + "  dataW/H:"
	 ...
	 */
	static class NgnProxyVideoConsumerGLCallback extends
		...
		public int prepare(int width, int height, int fps) {
			...
			NgnMediaPluginEventArgs.broadcastEvent(new NgnMediaPluginEventArgs(
					mConsumer.mId, NgnMediaType.Video,
					ret == 0 ? NgnMediaPluginEventTypes.PREPARED_OK
		...
		public int start() {
			...
			NgnMediaPluginEventArgs.broadcastEvent(new NgnMediaPluginEventArgs(
					mConsumer.mId, NgnMediaType.Video,
					ret == 0 ? NgnMediaPluginEventTypes.STARTED_OK
		...
		public int pause() {
			...
			NgnMediaPluginEventArgs.broadcastEvent(new NgnMediaPluginEventArgs(
					mConsumer.mId, NgnMediaType.Video,
					ret == 0 ? NgnMediaPluginEventTypes.PAUSED_OK
		...
		public int stop() {
			...
			NgnMediaPluginEventArgs.broadcastEvent(new NgnMediaPluginEventArgs(
					mConsumer.mId, NgnMediaType.Video,
					ret == 0 ? NgnMediaPluginEventTypes.STOPPED_OK
	 ...
	 * OpenGL Surface view
	 */
	static class NgnProxyVideoConsumerGLPreview extends GLSurfaceView implements
		...
		public NgnProxyVideoConsumerGLPreview(Context context,
				boolean fullScreenRequired, ByteBuffer buffer, int bufferWidth,
				int bufferHeight, int fps) {
			super(context);
			MyLog.d(TAG, "NgnProxyVideoConsumerGLPreview()");
			...
			mTriangleVertices = ByteBuffer
					.allocateDirect(
							TRIANFLE_VERTICES_DATA.length * FLOAT_SIZE_BYTES)
					.order(ByteOrder.nativeOrder()).asFloatBuffer();
			mTriangleVertices.put(TRIANFLE_VERTICES_DATA).position(0);

			mIndices = ByteBuffer
					.allocateDirect(INDICES_DATA.length * SHORT_SIZE_BYTES)
		...
		public void setBuffer(ByteBuffer buffer, int bufferWidth,
				int bufferHeight) {
			// MyLog.d(TAG, ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> setBuffer()");
			mBuffer = buffer;
			mBufferWidthY = bufferWidth;
			mBufferHeightY = bufferHeight;

			// MyLog.d(TAG, "1111 mBufferWidthY  = " + mBufferWidthY);
			// MyLog.d(TAG, "1111 mBufferHeightY  = " + mBufferHeightY);
			...
			// MyLog.d(TAG, "2222 mBufferWidthY  = " + mBufferWidthY);
			// MyLog.d(TAG, "2222 mBufferHeightY  = " + mBufferHeightY);
		...
		public void onDrawFrame(GL10 glUnused) {
			...
			if (mBuffer != null) {
				synchronized (this) {
					...
					GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0,
							GLES20.GL_LUMINANCE, mBufferWidthUV,
							mBufferHeightUV, 0, GLES20.GL_LUMINANCE,
		...
		public void onSurfaceChanged(GL10 glUnused, int width, int height) {
			/*
			 * GLES20.glViewport(0, 0, width, height); setViewport(width,
			 * height); // GLU.gluPerspective(glUnused, 45.0f,
			 * (float)width/(float)height, 0.1f, 100.0f);
			 * 
			 * GlobalVar.isVideoDisp = true;
			 */
		}
		...
		public void onSurfaceCreated(GL10 glUnused, EGLConfig config) {
			...
			muSamplerYHandle = GLES20
					.glGetUniformLocation(mProgram, "SamplerY");
			if (muSamplerYHandle == -1) {
				throw new RuntimeException(
						"Could not get uniform location for SamplerY");
			}
			muSamplerUHandle = GLES20
					.glGetUniformLocation(mProgram, "SamplerU");
			if (muSamplerUHandle == -1) {
				throw new RuntimeException(
						"Could not get uniform location for SamplerU");
			}
			muSamplerVHandle = GLES20
					.glGetUniformLocation(mProgram, "SamplerV");
			...
			if (mViewHeight != 0 && mViewWidth != 0) {
				...
				MyLog.d(TAG, "mViewHeight!=0 && mViewWidth!=0");
				setViewport(mViewWidth, mViewHeight);
			} else {
				MyLog.d(TAG, "mViewHeight or mViewWidth is 0:");
		...
		private void setViewport(int width, int height) {
			...
			/*
			 * if(mFullScreenRequired){ mViewWidth = width; mViewHeight =
			 * height; mViewX = mViewY = 0; } else{ float fRatio = ((float)
			 * mBufferWidthY / (float) mBufferHeightY); mViewWidth = (int)
			 * ((float) width / fRatio) > height ? (int) ((float) height *
			 * fRatio) : width; mViewHeight = (int) (mViewWidth / fRatio) >
			 * height ? height : (int) (mViewWidth / fRatio); mViewX = ((width -
			 * mViewWidth) >> 1); mViewY = ((height - mViewHeight) >> 1);
			 * 
			 * }
			 */

			MyLog.d(TAG, "VideoConsumer Preview1 W:" + width);
			MyLog.d(TAG, "VideoConsumer Preview1 H:" + height);

			MyLog.d(TAG, "VideoConsumer Preview1 W/H:" + (width + "/" + height));

			DisplayMetrics dm = new DisplayMetrics();
			WindowManager wm = (WindowManager) NgnSKDroid.getInstence().getGlobleContext()
					.getSystemService(Context.WINDOW_SERVICE);
			wm.getDefaultDisplay().getMetrics(dm);

			float fRatio = ((float) width / (float) height);
			mViewWidth = (int) ((float) dm.widthPixels / fRatio) > dm.heightPixels ? (int) ((float) dm.heightPixels * fRatio)
					: dm.widthPixels;
			mViewHeight = (int) (mViewWidth / fRatio) > dm.heightPixels ? dm.heightPixels
					: (int) (mViewWidth / fRatio);
			mViewX = ((dm.widthPixels - mViewWidth) >> 1);
			mViewY = ((dm.heightPixels - mViewHeight) >> 1);
			MyLog.d(TAG, "VideoConsumer Preview W/H:"
