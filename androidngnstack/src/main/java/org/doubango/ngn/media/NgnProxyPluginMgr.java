...
public class NgnProxyPluginMgr {
	...
	//add by gle 解决视频通话协商SDP问题
	public static void setDefaultMaxVideoSize()
	{
		MediaSessionMgr.defaultsSetPrefVideoSize(tmedia_pref_video_size_t.tmedia_pref_video_size_1080p);
	}
	public static void setDefaultMinVideoSize()
	{
		MediaSessionMgr.defaultsSetPrefVideoSize(tmedia_pref_video_size_t.tmedia_pref_video_size_cif);
	}
	public static void setCurrentVideoSize()
	{
		
		MediaSessionMgr.defaultsSetPrefVideoSize(tmedia_pref_video_size_t.valueOf(NgnEngine.getInstance().getConfigurationService().getString(
	...
	static class MyProxyPluginMgrCallback extends ProxyPluginMgrCallback
	{
		...
		public int OnPluginCreated(BigInteger id, twrap_proxy_plugin_type_t type) {
			MyLog.d(TAG, "OnPluginCreated("+id+","+ type+")");
			switch(type){
...
//							sPlugins.put(id, myProducer);
							...
							if (NgnApplication.getSDKVersion() >= 14) {
								...
								NgnProxyVideoProducer_surface myProducer_surface = new NgnProxyVideoProducer_surface(id, producer);
								sPlugins.put(id, myProducer_surface);
							}
						}
					}
					break;
		...
		public int OnPluginDestroyed(BigInteger id, twrap_proxy_plugin_type_t type) {
			MyLog.d(TAG, "OnPluginDestroyed("+id+","+ type+")");
			switch(type){
				case twrap_proxy_plugin_audio_producer:
				case twrap_proxy_plugin_video_producer:
				case twrap_proxy_plugin_audio_consumer:
